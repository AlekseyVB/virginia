...
OBJECTS_DATA_TEMPLATE = pd.DataFrame({'File': ['Настройки',DEFAULT_FILE_NAME],'Width':[3,MAX_WIDTH],'Height':['red',MAX_HEIGHT],'Объект 1':['green',[0,0,0,0]]})
#OBJECTS_DATA_TEMPLATE = pd.DataFrame({'File': ['Настройки',DEFAULT_FILE_NAME],'Width':[3,MAX_WIDTH],'Height':['red',MAX_HEIGHT],'Объект 1':['blue',[0,0,0,0]],'Объект 2':['green',[0,0,0,0]],'Объект 3':['Orange',[0,0,0,0]]})
ObjectsData = OBJECTS_DATA_TEMPLATE.copy()
...
def Create_Window():
  ...
  layout = [
            ...
            [sg.Push(),
             ...
             sg.VerticalSeparator(pad=None),
             sg.Text('Смещение: dX'),
             sg.Input(dx, key = 'delta_x', size = (3, 1), enable_events= True, justification = 'center', focus = 'False', disabled = MoveOff),
             sg.Button('+',size = (1, 1), key = 'delta_xu', disabled = MoveOff),
             sg.Button('-',size = (1, 1), key = 'delta_xd', disabled = MoveOff),
             sg.Text('dY'),
             sg.Input(dy, key = 'delta_y', size = (3, 1), enable_events= True, justification = 'center', focus = 'False'),
             sg.Button('+',size = (1, 1), key = 'delta_yu', disabled = MoveOff),
             sg.Button('-',size = (1, 1), key = 'delta_yd', disabled = MoveOff),
             sg.VerticalSeparator(pad=None),
             sg.Text('Авто:'),
             ...
             sg.Checkbox('Центрирование',  key = 'auto_mov', enable_events= True, default = AutoMove, disabled = MoveOff),
             sg.Push()
            ],
            [sg.Push(),
             sg.Graph(
...
def Redraw_Objects():
  ...
  for i in range(3, MaxObjects+3):
    # Рамки рисуются наружу от сохраненных координат, т.е. весь объет лежит внутри рамки
    rectangle = graph.draw_rectangle((ObjectsData[ObjectsData['File'] == FileName].iloc[0,i][0]-dx-line_width+line_width//2,
                                      ObjectsData[ObjectsData['File'] == FileName].iloc[0,i][1]-dy-line_width+line_width//2),
                                     (ObjectsData[ObjectsData['File'] == FileName].iloc[0,i][2]-dx+line_width//2+1,
                                      ObjectsData[ObjectsData['File'] == FileName].iloc[0,i][3]-dy+line_width//2+1),
                                      fill_color = None,
...
def Redraw_Frame():
  ...
  global width, height, img_width, img_height, height, data, window, graph, dx, dy, MoveOff, AutoMove
  ...
  if ObjectsData['Width'].iloc[FileNumber] != img_width or ObjectsData['Height'].iloc[FileNumber] != img_height:
    ...
    # Закрывается и заново создается окно
    window.close()
    Create_Window()
    ...
    if AutoMove:
      dx = (img_width - MAX_WIDTH)//2
      dy = (img_height - MAX_HEIGHT)//2
    
    graph.erase()
    Update_Obj_Info()

  # Перенос изображения из файла на graph
...
def Update_Obj_Info():
  ...
  window['delta_x'].update(dx)
  window['delta_y'].update(dy)
...
def Update_DataFile(ObjectsData, FilesList, Directory, FileName):
  ...
  ObjectsData = ObjectsData[ObjectsData['File'] != DEFAULT_FILE_NAME]
...
# Рисую пустую линию для дальнейшей корректрой обработки движения перекрестия
...
while True:
  ...
  if event == 'graph_mouse_motion':
    ...
    # Обработка меню
        ...
        if len(Path) != 0:
          Action = sg.popup_yes_no('Существуют данные обработки папки:\n'+ str(Directory)+'\nСохранить?')
          if Action != 'No': Save_DataFile()
        ...
        if len(Path) != 0: #Проверка что файл был выбран, а не просто закрыто окно
          ...
          if FileName == DEFAULT_FILE_NAME:   # Страховка от выбора пользователем пустого файла
            sg.popup('Изображение "Пустой.jpg" является временный файлом программы!\nПожалуйста, выберите другой файл.')
          else:
            # Удаление временного файла на случай если скрипт лежит в папке с изображениями
      ...
      if len(Path) == 0:
        sg.popup('Выберите сначала папку с изображениями, пожалуйста.')
      else:
        Save_DataFile()
        sg.popup('Файл "FORA_ObjectsData.pickle" с данными об изображениях и объектах сохранен в папке:\n' + str(Directory))
    ...
    # Обработка полей ввода и кнопок
      ...
      AutoFrames = values["auto_fra"]

    elif event == 'auto_mov':
      AutoMove = values["auto_mov"]
      if AutoMove: Redraw_Frame()

    elif event == 'delta_x':
      while event == 'delta_x':
        event, values = window.read()
      try:
        New_dx = int(values['delta_x'])
        dx = New_dx
        AutoMove = False
        window['auto_mov'].update(AutoMove)
      except:
        window['delta_x'].update(dx)
        sg.popup('Введите целое число!')        
      Redraw_Frame()
    ...
    # Обработка действия мыши 
      ...
      if Marking:
        ...
        # Создание прямоугольника для дальнейшей работы
        x,y = values['graph']
        rectangle = graph.draw_rectangle((Rect_x0, Rect_y0), (x, y),
                                         fill_color = None,
                                         line_color = ObjectsData.iloc[0,3+Current],
      ...
      if Marking:
        ...
        # Вычисления правильных координат внутри рамки в зависимости от толщины линий для сохранения
        if Rect_x0 < x:
          ObjectsData.iloc[FileNumber,Current+3][0] = Rect_x0+dx+line_width-line_width//2
          ObjectsData.iloc[FileNumber,Current+3][2] = x+dx-line_width//2-1
        else:
          ObjectsData.iloc[FileNumber,Current+3][2] = Rect_x0+dx-line_width+line_width//2
          ObjectsData.iloc[FileNumber,Current+3][0] = x+dx+line_width//2+1
        if Rect_y0 < y:
          ObjectsData.iloc[FileNumber,Current+3][1] = Rect_y0+dy+line_width-line_width//2
          ObjectsData.iloc[FileNumber,Current+3][3] = y+dy-line_width//2-1
        else:
          ObjectsData.iloc[FileNumber,Current+3][3] = Rect_y0+dy-line_width+line_width//2
          ObjectsData.iloc[FileNumber,Current+3][1] = y+dy+line_width//2+1
      ...
      # Поскольку при нажатии объекты очищаются, то когда отпускается кнопка, нужно все восстановить
      Redraw_Objects()        
      x,y = values['graph']
      v_line = graph.draw_line((x,0),(x,height), width = ObjectsData['Width'].iloc[0], color=ObjectsData['Height'].iloc[0])
      h_line = graph.draw_line((0,y),(width,y), width = ObjectsData['Width'].iloc[0], color=ObjectsData['Height'].iloc[0])

    # Drag_n_drop (расширение функционала на будущее)
